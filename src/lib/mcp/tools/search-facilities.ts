...
export default defineTool({
  ...
  description:
    "Search facility permit records. Filter by name (partial, case-insensitive), sector, district, region_id, or office_id. Returns up to `limit` rows (default 25, max 200).",
  inputSchema: {
  ...
  handler: async ({ name, sector, district, region_id, office_id, limit }) => {
    ...
    if (sector) q = q.eq("sector", sector as any);
